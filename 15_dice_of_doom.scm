...
(define (gen-board)
  (board-array (list-ec (: i *board-hexnum*) (list (random-integer *num-player*) (+ 1 (random-integer *max-dice*)))))
  )

(define (player-letter p)
  (integer->char (+ p 97))
  )
...
(define (draw-board board)
  (do-ec (: i *board-size*)
    (begin
      (newline)
      (do-ec (: j (- *board-size* i)) (display "  "))
      (do-ec (: k *board-size*)
        (:let hex (~ board (+ k (* *board-size* i))))
        (format #t "~a-~a "
...
(define (winners board)
  (let* ([tally (list-ec (: hex board) (car hex))]
         [totals (map (^[player] (cons player (count (cut eq? player <>) tally))) (delete-duplicates tally))]
...
(define (get-ratings tree player)
  (map (^m (rate-position (cadr m) player))
       (caddr tree)
       )
  )
...
(let ((old-neighbors neighbors)
  ...
  (set! neighbors 
    (^p 
      (or (hash-table-get cache p #f)
        (rlet1 v (old-neighbors p)
          (hash-table-put! cache p v)
          ))))
  )
  ...
  (define (f lst n acc)
    (cond 
      ...
      [else (let ([cur-player (caar lst)]
                  [cur-dice (cadar lst)])
              (if (and (eq? cur-player player) (< cur-dice *max-dice*))
                (f (cdr lst) (- n 1) (cons (list cur-player (+ 1 cur-dice)) acc))
